...
$_SESSION['$nom']=$_POST['pasword'];
$sum=mysqli_connect($server,$utilisateur,$motdepasse,$base);
if(isset($_POST['Connexion'])){
   $vide=null; 
   $test_num_etd=mysqli_query($sum,"SELECT * FROM joueur WHERE Id_jou='".$_SESSION['$num_etd']."'");
   $resultat_test_num_etd=mysqli_num_rows($test_num_etd);
      if($resultat_test_num_etd>0){
         ...
         if($resultat_test_mdp_etd>0){
            ...
            $resultat = mysqli_query($sum,"SELECT Id_jou FROM joueur WHERE Id_jou = '".$_SESSION['$num_etd']."' AND pasword = '".$_SESSION['$mdp_adm']."' ");
            ...
            $resultat3 = mysqli_query($sum,"SELECT images FROM joueur WHERE Id_jou = '".$_SESSION['$num_etd']."' AND pasword = '".$_SESSION['$mdp_adm']."' ");
            // Extraire la valeur unique de résultat
         ...
         $_SESSION['table_images'] = $table_images;*/
            ...
            header("Location:../connexion/connexion1.php");
            }else {
...
if(isset($_POST['Enregistrer'])){
   ...
   $resultat = mysqli_query($sum, "SELECT * FROM joueur WHERE Id_jou='$num_etd'");
   if(mysqli_num_rows($resultat) == 1){ 
       // Mise à jour du mot de passe si les deux nouveaux mots de passe sont identiques
       if($password == $password1 ){
           mysqli_query($sum,"UPDATE joueur SET pasword='$password' WHERE Id_jou='$num_etd'");
